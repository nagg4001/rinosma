  ...
  return (
    <ErrorBoundary FallbackComponent={ErrorFallback}>
      <Box maxW="2xl" mx="auto" p={6} borderWidth="1px" borderRadius="md" boxShadow="md">
        ...
        <Box mt={4}>
          {renderSelect("??????", deductionOptions.frameScratch, "??????")}
          {renderSelect("????", deductionOptions.screenScratch, "????")}
          {renderSelect("??????????", deductionOptions.networkLimitation, "??????????")}
          {renderSelect("??????", deductionOptions.cameraStain, "??????")}
          {renderSelect("????", deductionOptions.malfunction, "????")}
        ...
        <Box mt={6} textAlign="center">
          ...
          <Text>????: ?{taxExclusivePrice.toLocaleString()}</Text>
          <Text>????10%?: ?{consumptionTax.toLocaleString()}</Text>
        </Box>
      </Box>
    </ErrorBoundary>
  )
